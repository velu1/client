...
const PartsInPage = () => {
  const [mode, setMode] = useState<"ai" | "manual">(() => {
    return (localStorage.getItem("partsInMode") as "ai" | "manual") || "ai";
  });
  const [pendingMode, setPendingMode] = useState<"ai" | "manual" | null>(null);
  const [showModeDialog, setShowModeDialog] = useState(false);

  const requestModeSwitch = (next: "ai" | "manual") => {
    if (next === mode) return;
    setPendingMode(next);
    ...
    <div className="px-4 w-full">
      {showModeDialog && (
        <DialogComponent
          ...
          onClose={() => { setPendingMode(null); setShowModeDialog(false); }}
          title="Switch mode"
          subtitle={
            pendingMode === "manual"
      ...
      <div className="my-5 space-y-3">
        <PartsInCapture
          isActive={mode === "ai"}
          onActivate={() => requestModeSwitch("ai")}
        ...
        <PartsInManual
          ...
          onActivate={() => requestModeSwitch("manual")}
          invoiceMode={!!printerConfig?.invoice}
          invoiceOptions={invoiceOptions}
          invoiceLoading={invoiceLoading}
          onInvoiceSelect={handleInvoiceSelect}
