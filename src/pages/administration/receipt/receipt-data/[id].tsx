...
import { getAllTemplate } from "../../../../api/administration/receipt";
...
const ReceiptDataDetailPage = () => {
  ...
  const fetchTemplates = async () => {
    try {
      setLoading(true);
      const response = await getAllTemplate("invoice");
  ...
  const excelDateToJSDate = (serial: number): string => {
    const excelEpoch = new Date(Date.UTC(1899, 11, 30)); // Excel's start date
    const days = Math.floor(serial);
    const date = new Date(excelEpoch.getTime() + days * 86400 * 1000);

    return date.toISOString().split("T")[0]; // Returns in "YYYY-MM-DD"
  };
    ...
    try {
      ...
      reader.onload = async (e: ProgressEvent<FileReader>) => {
        try {
          ...
          // Set file data for display (start from header row)
    ...
    try {
      ...
      reader.onload = async (e: ProgressEvent<FileReader>) => {
        try {
          ...
          const rows = jsonData.slice(Number(headerRowNo)) as string[][];
          ...
          const filteredParsedObjects = parsedObjects.filter((obj) => {
          ...
          console.log("Filtered parsedObjects", filteredParsedObjects);

          // Use filteredParsedObjects instead of parsedObjects for validation
          requiredKeys.forEach((key) => {
            const hasKey = filteredParsedObjects.some(
              (obj) =>
          ...
          // If any required keys are missing, show error and return
          if (missingKeys.length > 0) {
            toast.error(`Missing required columns: ${missingKeys.join(", ")}`);
            return;
          }

          const payload = {
            type: "upload",
            uploadData: filteredParsedObjects, // Use filtered data here
          };

          const response = await uploadTemplateData(payload, "invoice");
...
export default ReceiptDataDetailPage;
